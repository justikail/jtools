...
function CSR() {
  ...
  const handleSubmit = async (e) => {
    ...
    setResult(null);
    setError(null);
    const response = await fetch("/api/length-url", {
      method: "POST",
      headers: {
  ...
  return (
    <CardTools title="URL Lengthener" description="Tools for generate long url to more long url :D.">
      <form onSubmit={handleSubmit} className="p-1 md:p-4 flex flex-col gap-4 w-full">
        ...
        <BtnSubmit isDisabled={isDisabled} loading={loading} innerText="Short" />
      </form>
